...
router.get('/detail/:userId', (req, res) => {
  ...
  dbUsers.findOne(
    ...
    .then(_userInfo => {
      if (!_userInfo) throw Error('Cannot find user info.');
      userInfo = _userInfo;
      return dbUserDetail.findOne({ userId: userId });
    })
    .then(userDetails => {
      if (!userDetails) throw Error('Cannot find user details.');
